...
import { CANCEL_ORDER, CREATE_ORDER, GET_DEPTH, GET_OPEN_ORDERS, GET_TICKERS, MessageFromApi, ON_RAMP, Ticker } from "../types/apiMessage";
...
export class Engine {
    ...
    constructor() {
        ...
        if (snapshot) {
            ...
            this.orderbooks = snapshotSnapshot.orderbooks.map((o: any) => new Orderbook(o.baseAsset, o.bids, o.asks, o.lastTradeId, o.currentPrice, o.trades));
        ...
        switch (message.type) {
            ...
            case CANCEL_ORDER:
                try {
                    ...
                    const cancelOrderbook = this.orderbooks.find(o => o.getSymbol() === cancelMarket);
            ...
            case GET_OPEN_ORDERS:
                try {
                    const openOrderbook = this.orderbooks.find(o => o.getSymbol() === message.data.market);
                    if (!openOrderbook) {
                        throw new Error("No orderbook found");
                    }
            ...
            case GET_DEPTH:
                try {
                    ...
                    const orderbook = this.orderbooks.find(o => o.getSymbol() === market);
                    if (!orderbook) {
                        throw new Error("No orderbook found");
                    }
                    RedisManager.getInstance().sendToApi(clientId, {
                ...
                break;
            case GET_TICKERS:
                const tickers: Ticker[] = this.orderbooks.map(o => o.ticker());
                RedisManager.getInstance().sendToApi(clientId, {
                    type: "TICKERS",
                    payload: tickers
                });
    ...
    createOrder(market: string, price: string, quantity: string, side: "buy" | "sell", userId: string) {
        ...
        this.publishWsTickerUpdates(market)
    ...
    publisWsDepthUpdates(fills: Fill[], price: string, side: "buy" | "sell", market: string) {
        ...
        if (side === "buy") {
            ...
            fills.forEach(fill => {
                const askExists = updatedAsks.some(ask => ask[0] === fill.price.toString());
                if (!askExists) {
                    updatedAsks.push([fill.price.toString(), '0.00']);
                }
            });
        ...
        if (side === "sell") {
            ...
            fills.forEach(fill => {
                const bidExists = updatedBids.some(bid => bid[0] === fill.price.toString());
                if (!bidExists) {
                    updatedBids.push([fill.price.toString(), '0.00']);
                }
            });
    ...
    publishWsTickerUpdates(market: string) {
        const orderbook = this.orderbooks.find(o => o.getSymbol() === market);
        if (!orderbook) {
            return;
        }
        const ticker = orderbook.ticker();
